...
export function validateConfig(partial: Partial<Config>): Config {
    ...
    // Validate update settings (T023)
    if (typeof partial.checkForUpdates === 'boolean') {
        config.checkForUpdates = partial.checkForUpdates;
    }
    if (partial.lastUpdateCheck === null || typeof partial.lastUpdateCheck === 'string') {
        config.lastUpdateCheck = partial.lastUpdateCheck;
    }
